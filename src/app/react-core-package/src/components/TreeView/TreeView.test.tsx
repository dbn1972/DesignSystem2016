...
describe('TreeView', () => {
  ...
  describe('Accessibility', () => {
    it('has no axe violations in default state', async () => {
      await assertA11y(<TreeView nodes={nodes} />);
    });

    describe('Keyboard navigation', () => {
      it('navigates tree items via click interaction', async () => {
      ...
      it('expands and collapses node via click', async () => {
        const user = userEvent.setup();
        render(<TreeView nodes={nodes} />);
        await user.click(screen.getByText('Documents'));
        expect(screen.getByText('Aadhaar.pdf')).toBeInTheDocument();
        await user.click(screen.getByText('Documents'));
        expect(screen.queryByText('Aadhaar.pdf')).not.toBeInTheDocument();
      });

      it('selects leaf node via click', async () => {
        const user = userEvent.setup();
