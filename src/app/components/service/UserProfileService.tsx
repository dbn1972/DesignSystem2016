            ...
            {activeTab === "profile" && (
              <div className="bg-white border-2 border-gray-300 rounded-lg overflow-hidden">
                ...
                <div className="p-6 space-y-6">
                  ...
                  {editMode && (
                    <div className="pt-4 border-t-2 border-gray-200 flex items-center gap-3">
                      <button type="button" onClick={handleSave} className="px-5 py-2 bg-[#138808] text-white rounded font-bold text-sm hover:bg-green-700">Save Changes</button>
                      <button type="button" onClick={() => setEditMode(false)} className="px-5 py-2 border-2 border-gray-300 rounded font-bold text-sm text-gray-700 hover:bg-gray-50">Cancel</button>
                    </div>
                  )}
                </div>
              </div>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-700">Update your password regularly to keep your account secure.</p>
                      </div>
                    </div>
                    <Link to={forgotPasswordPath} className="px-4 py-2 bg-[#000080] text-white rounded font-bold text-sm hover:bg-blue-900 whitespace-nowrap">Change Password</Link>
                  </div>
                </div>
            ...
            {activeTab === "notifications" && (
              <div className="bg-white border-2 border-gray-300 rounded-lg p-6 space-y-4">
                ...
                <div className="pt-4 border-t-2 border-gray-200">
                  <button type="button" onClick={handleSave} className="px-5 py-2 bg-[#138808] text-white rounded font-bold text-sm hover:bg-green-700">Save Preferences</button>
                </div>
              </div>
            )}

            <div className="flex items-center gap-4 text-sm">
              <Link to={myApplicationsPath} className="font-bold text-[#000080] hover:underline">My Applications</Link>
              {codeDownloadPath && (
...
function TabButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`w-full px-4 py-3 flex items-center gap-2 text-left border-b-2 border-gray-200 ${
        active ? "bg-[#000080] text-white" : "bg-white text-gray-700 hover:bg-gray-50"
      }`}
    >
      {icon}
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`w-full pl-9 pr-4 py-2.5 border-2 rounded text-sm ${
            disabled ? "border-gray-300 bg-gray-50 text-gray-700" : "border-gray-300 bg-white text-gray-900"
          }`}
        />
      </div>
    </div>
...
function NotificationToggle({
  ...
  return (
    <div className="flex items-start justify-between gap-3 p-4 border-2 border-gray-300 rounded">
      <div>
      ...
      <label className="relative inline-block w-11 h-6 flex-shrink-0">
        ...
        <span className={`absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform ${checked ? "translate-x-5" : ""}`} />
      </label>
    </div>
  );
}
