...
import {  Shield, Smartphone, Mail, Lock, Key, RefreshCw, UserCheck, Clock, AlertTriangle, CheckCircle, ArrowRight, FileText, Info, Eye, Users, Layers, GitBranch, Database, Zap, Globe, Code, BookOpen, Settings, BarChart3, XCircle, Download, Copy, Check } from "lucide-react";
    ...
    <section className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {flows.map((flow) => (
          ...
          >
            {/* Icon Header */}
            <div className="bg-background border-b-2 border-border p-6 h-28 flex items-center justify-center group-hover:bg-blue-50 dark:bg-blue-900/20 transition-colors">
              {flow.icon}
            </div>
            ...
            <div className="p-4">
              <div className="flex items-start justify-between mb-2">
                ...
                <span className="px-2 py-0.5 bg-green-100 text-green-700 dark:text-green-400 text-xs font-bold rounded-full flex-shrink-0 ml-2">
                  {flow.status}
                </span>
              </div>
  ...
  return (
    <tr className="border-b border-border hover:bg-background">
      ...
      <td className="py-3 px-4">
        <span className="px-2 py-1 bg-green-100 text-green-700 dark:text-green-400 text-xs font-bold rounded-full">
          {status}
        </span>
      </td>
...
const IDENTITYACCESS_REACT_CODE = `import React, { useState } from 'react';
...
export default function IdentityAccessPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const res = await fetch('/api/identity-access', {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <div className="w-full max-w-lg bg-card border border-border rounded-2xl p-8 shadow-sm">
        <h1 className="text-2xl font-bold text-foreground mb-2">Identity Access</h1>
...
const IDENTITYACCESS_ANGULAR_CODE = `import { Component } from '@angular/core';
import { CommonModule } from '@angular/common';

@Component({
  selector: 'ux4g-identity-access',
  standalone: true,
  imports: [CommonModule],
  template: \`
    <div class="min-h-screen flex items-center justify-center bg-background p-4">
      <div class="w-full max-w-lg bg-card border border-border rounded-2xl p-8 shadow-sm">
        <h1 class="text-2xl font-bold text-foreground mb-2">Identity Access</h1>
...
const IDENTITYACCESS_HTML_CODE = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Identity Access — UX4G</title>
  <style>
...
function IdentityAccessCodeDownloads() {
  const [copiedId, setCopiedId] = useState<string | null>(null);
  const copyToClipboard = (code: string, id: string) => { navigator.clipboard.writeText(code); setCopiedId(id); setTimeout(() => setCopiedId(null), 2000); };
  const downloadCode = (code: string, filename: string) => { const blob = new Blob([code], { type: 'text/plain' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = filename; a.click(); URL.revokeObjectURL(url); };
  const lanes = [
    { key: 'react', title: 'React', desc: 'TypeScript + Hooks', code: IDENTITYACCESS_REACT_CODE, filename: 'IdentityAccessPage.tsx' },
    { key: 'angular', title: 'Angular', desc: 'Standalone Component', code: IDENTITYACCESS_ANGULAR_CODE, filename: 'identity-access.component.ts' },
    { key: 'html', title: 'HTML / CSS / JS', desc: 'No framework needed', code: IDENTITYACCESS_HTML_CODE, filename: 'identity-access.html' },
  ];
  return (
    <section id="code-downloads" className="space-y-6 scroll-mt-24 mt-12">
      <div className="border-l-4 border-primary pl-4">
        <h2 className="text-2xl font-bold text-foreground">Code Downloads</h2>
        <p className="text-muted-foreground mt-1">Production-ready Identity Access implementations.</p>
      </div>
