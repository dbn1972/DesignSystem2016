...
const ACCORDION_PLAYGROUND_CONTROLS: PlaygroundControl[] = [
  { name: 'allowMultiple', label: 'Allow Multiple', type: 'boolean', defaultValue: false },
  { name: 'bordered', label: 'Bordered', type: 'boolean', defaultValue: true },
  { name: 'defaultOpen', label: 'First item open', type: 'boolean', defaultValue: true },
];
...
function AccordionPlayground() {
  return (
    <ComponentPlayground
      name="Accordion"
      controls={ACCORDION_PLAYGROUND_CONTROLS}
      renderPreview={() => (
      ...
      codeTemplate={(v) =>
        `<Accordion${v.allowMultiple ? ' allowMultiple' : ''}${v.bordered ? ' bordered' : ''}${v.defaultOpen ? ' defaultOpen' : ''} />`
      }
    />
  );
}
