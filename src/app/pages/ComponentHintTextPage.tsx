...
const HINTTEXT_PLAYGROUND_CONTROLS: PlaygroundControl[] = [
  { name: 'visible', label: 'Visible', type: 'boolean', defaultValue: false },
  { name: 'icon', label: 'Icon', type: 'boolean', defaultValue: false },
  { name: 'message', label: 'Message', type: 'text', defaultValue: 'Enter your 12-digit Aadhaar number' },
];
...
function HintTextPlayground() {
  return (
    <ComponentPlayground
      name="HintText"
      controls={HINTTEXT_PLAYGROUND_CONTROLS}
      renderPreview={(v) => (
        <div className="w-full flex items-center justify-center">
          <HintTextPreview icon={v.icon}>{v.message}</HintTextPreview>
        </div>
      )}
      codeTemplate={(v) =>
        `<HintText${v.visible ? ' visible' : ''} />`
      }
    />
  );
}
