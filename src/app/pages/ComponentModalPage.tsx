...
const MODAL_PLAYGROUND_CONTROLS: PlaygroundControl[] = [
  ...
  { name: 'showClose', label: 'Show close button', type: 'boolean', defaultValue: true },
];
      ...
      renderPreview={(v) => (
        <div className="w-full max-w-sm bg-card border border-border rounded-xl shadow-lg">
          <div className="flex items-center justify-between p-4 border-b border-border">
            <h3 className="font-semibold text-foreground text-sm">{v.size === 'lg' ? 'Document Preview' : 'Confirm Action'}</h3>
            {v.showClose && <span className="text-muted-foreground cursor-pointer">✕</span>}
          </div>
          <div className="p-4">
            <p className="text-sm text-muted-foreground">Modal content goes here.</p>
          </div>
          <div className="flex justify-end gap-2 p-4 border-t border-border">
            <button className="px-3 py-1.5 text-xs border border-border rounded">Cancel</button>
            <button className="px-3 py-1.5 text-xs bg-[#005196] text-white rounded">Confirm</button>
          </div>
        </div>
      )}
      codeTemplate={(v) =>
