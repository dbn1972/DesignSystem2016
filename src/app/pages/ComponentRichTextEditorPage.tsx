...
const RICHTEXTEDITOR_CONTROLS: PlaygroundControl[] = [
  {
    name: 'showToolbar',
    label: 'Show Toolbar',
    type: 'boolean',
...
function RichTextEditorPlayground() {
  return (
    <ComponentPlayground
      name="RichTextEditor"
      controls={RICHTEXTEDITOR_CONTROLS}
      renderPreview={(v) => (
        <div className="w-full max-w-lg">
          <RichTextEditorPreview {...v} />
        </div>
      )}
      codeTemplate={(v) => {
        const props: string[] = [];
        RICHTEXTEDITOR_CONTROLS.forEach((c) => {
          const val = v[c.name];
        ...
        return `<RichTextEditor${props.length ? ' ' + props.join(' ') : ''} />`;
      }}
    />
  );
}
