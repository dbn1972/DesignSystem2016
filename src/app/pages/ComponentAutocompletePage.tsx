    ...
    <div className="relative w-full max-w-md" ref={wrapperRef}>
      <div className={'relative flex flex-col gap-2 ' + (multiple ? 'min-h-[44px]' : '')}>
        <div className={'flex items-center gap-2 rounded border border-border bg-card ' + (disabled ? 'opacity-60 cursor-not-allowed bg-background' : 'cursor-text')}>
          ...
          <button
            type="button"
            className="px-2 text-gray-400 hover:text-muted-foreground"
          ...
          >
            {groupedOptions ? (
              groupedOptions.map((group, groupIndex) => (
                <div key={groupIndex}>
                  <div className="px-3 py-2 text-xs font-semibold text-gray-500 bg-background border-b">
                    {group.group}
                  </div>
...
export default function ComponentAutocompletePage() {
  return (
    <ComponentDocumentation
      ...
      preview={
        <div className="text-center space-y-3">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-[#005196]/10 text-[#005196] text-2xl font-bold">Au</div>
          <p className="text-sm text-muted-foreground">Autocomplete component in its default state</p>
        </div>
      }
      ...
      props={[
        {
          ...
          type: 'Array<any>',
          required: true,
          description: 'Array of options to display in the dropdown. Can be primitive values or objects with id and name properties.',
        },
        {
          name: 'value',
          type: 'any | Array<any>',
          required: false,
          description: 'Selected value(s). Single value for single-select mode, array for multi-select mode.',
        },
        {
          name: 'onChange',
          type: '(value: any | Array<any>) => void',
          required: true,
          description: 'Callback fired when selection changes. Receives the new selected value(s).',
        },
        {
          name: 'onSearch',
          type: '(query: string) => void | Promise<void>',
          required: false,
          description: 'Callback fired when search query changes. Used for async data fetching or custom filtering.',
        },
        {
          name: 'placeholder',
          type: 'string',
        ...
        },
        {
          ...
          default: 'false',
          required: false,
          description: 'Enable multi-select mode. Allows selecting multiple options with tag display.',
        },
        {
          name: 'loading',
          type: 'boolean',
          default: 'false',
          required: false,
          description: 'Loading state for async operations. Shows loading spinner and prevents interaction.',
        },
        {
          name: 'disabled',
          type: 'boolean',
          default: 'false',
          required: false,
          description: 'Disable the autocomplete. Prevents interaction and applies disabled styling.',
        },
        {
          name: 'size',
          type: "'sm' | 'md' | 'lg'",
          default: "'md'",
          required: false,
          description: 'Size of the autocomplete input. Small (32px), Medium (40px), Large (48px).',
        },
        {
          name: 'renderOption',
          type: '(option: any) => ReactNode',
          required: false,
          description: 'Custom render function for options. Allows displaying additional information or custom layouts.',
        },
        {
          name: 'groupBy',
          type: '(option: any) => string',
          required: false,
          description: 'Function to group options by category. Returns the group label for each option.',
        },
        {
          name: 'filterOptions',
          type: '(options: Array<any>, query: string) => Array<any>',
          required: false,
          description: 'Custom filter function. Defaults to case-insensitive substring matching.',
        },
        {
          name: 'noOptionsText',
          type: 'string',
        ...
        },
        {
          ...
          default: "'Loading...'",
          required: false,
          description: 'Text displayed during async loading.',
        },
        {
          name: 'clearable',
          type: 'boolean',
