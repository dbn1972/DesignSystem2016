...
const CALENDARSCHEDULER_CONTROLS: PlaygroundControl[] = [
  {
    name: 'view',
    label: 'View',
    type: 'text',
...
function CalendarSchedulerPlayground() {
  return (
    <ComponentPlayground
      ...
      controls={CALENDARSCHEDULER_CONTROLS}
      renderPreview={(v) => (
        <div className="w-full max-w-lg">
          <CalendarSchedulerPreview {...v} />
        </div>
      )}
      codeTemplate={(v) => {
        const props: string[] = [];
        CALENDARSCHEDULER_CONTROLS.forEach((c) => {
          const val = v[c.name];
        ...
        return `<CalendarScheduler${props.length ? ' ' + props.join(' ') : ''} />`;
      }}
    />
  );
}
