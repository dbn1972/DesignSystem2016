...
export default function MSMERegOfficerDashboard() {
  const stats = {
    totalCases: 120,
    pendingReview: 5,
    underReview: 3,
  ...
  const recentActivity = [
    { action: "Udyam Number Issued", refNumber: "MSME-2026-UDY-876543", applicant: "Priya Sharma", time: "5 minutes ago", type: "success" as const },
    { action: "Re-classification Updated", refNumber: "MSME-2026-UDY-875421", applicant: "Rajesh Kumar", time: "15 minutes ago", type: "warning" as const },
    { action: "New Registration", refNumber: "MSME-2026-UDY-874112", applicant: "Anita Desai", time: "30 minutes ago", type: "info" as const },
    { action: "Certificate Downloaded", refNumber: "MSME-2026-UDY-873098", applicant: "Vikram Singh", time: "1 hour ago", type: "success" as const },
  ];

  const urgentCases = [
    { refNumber: "MSME-2026-UDY-872567", applicant: "Meera Patel", classification: "Small", daysWaiting: 3 },
    { refNumber: "MSME-2026-UDY-871234", applicant: "Arjun Reddy", classification: "Medium", daysWaiting: 2 },
    { refNumber: "MSME-2026-UDY-870987", applicant: "Kavita Joshi", classification: "Micro", daysWaiting: 1 },
  ];
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-12 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-sm text-muted-foreground">DIC Officer Portal</div>
                <h1 className="text-2xl font-bold text-foreground">MSME/Udyam Registration Dashboard</h1>
              </div>
            </div>
            <div className="text-right">
              <div className="text-sm text-muted-foreground">Logged in as</div>
              <div className="font-bold text-foreground">Officer Sunita Rao</div>
              <div className="text-xs text-muted-foreground">District Industries Centre, Pune</div>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-12 py-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <StatCard icon={<Briefcase size={28} />} label="Total Registrations" value={stats.totalCases} color="blue" trend="+45 today" />
          <StatCard icon={<Clock size={28} />} label="Pending Review" value={stats.pendingReview} color="orange" trend="Minimal backlog" />
          <StatCard icon={<CheckCircle size={28} />} label="Completed Today" value={stats.completedToday} color="green" trend="Excellent throughput!" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
          ...
          <MetricCard icon={<Clock size={24} />} label="Avg. Processing" value={stats.avgProcessingTime} subtext="Fully automated" color="blue" />
          <MetricCard icon={<Calendar size={24} />} label="Under Review" value={stats.underReview.toString()} subtext="Manual verification" color="purple" />
        </div>
            ...
            <div className="bg-card border-2 border-red-300 rounded-lg overflow-hidden">
              <div className="bg-red-50 dark:bg-red-950/30 border-b-2 border-red-300 px-6 py-4">
                <h3 className="font-bold text-foreground flex items-center gap-2">
                  <AlertCircle size={18} className="text-red-600" /> Pending Cases
                </h3>
              </div>
              <div className="p-6 space-y-4">
                {urgentCases.map((c, i) => (
                  <div key={i} className="bg-red-50 dark:bg-red-950/30 border-2 border-red-300 rounded p-4">
                    ...
                    <div className="text-xs text-muted-foreground mb-2">{c.classification} Enterprise</div>
      ...
      <footer className="bg-card border-t-2 border-border mt-24">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-12 py-8">
          <div className="flex items-center justify-between text-sm text-muted-foreground">
            <div>MSME/Udyam Registration Service • DIC Officer Portal</div>
            <div>Dashboard</div>
          </div>
        </div>
      </footer>
