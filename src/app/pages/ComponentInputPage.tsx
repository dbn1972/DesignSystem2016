...
const INPUT_PLAYGROUND_CONTROLS: PlaygroundControl[] = [
  { name: 'type', label: 'Type', type: 'select', defaultValue: 'text', options: ['text', 'email', 'password', 'tel', 'search', 'number', 'url'] },
  { name: 'placeholder', label: 'Placeholder', type: 'text', defaultValue: 'Enter your name' },
...
function InputPlayground() {
  return (
    <ComponentPlayground
      name="Input"
      controls={INPUT_PLAYGROUND_CONTROLS}
      renderPreview={(v) => (
      ...
      codeTemplate={(v) =>
        `<Input type="${v.type}" placeholder="${v.placeholder}"${v.v.disabled ? ' disabled' : ''}${v.v.error ? ' error' : ''}${v.v.required ? ' required' : ''} />`}
    />
  );
}
