  ...
  const renderTreeNode = (node: TreeNode, level: number = 0) => {
    ...
    return (
      <div key={node.key} role="treeitem" aria-expanded={hasChildren ? isExpanded : undefined} aria-level={level + 1} aria-selected={isSelected}>
        <div
          className={'flex items-center gap-2 px-2 rounded cursor-pointer hover:bg-muted transition-colors ' + sizeClasses[size] + (isSelected ? ' bg-blue-50 dark:bg-blue-950/30 border-l-2 border-[#005196]' : '')}
...
const TREEVIEW_CONTROLS: PlaygroundControl[] = [
  {
    name: 'expandAll',
    label: 'Expand All',
    type: 'boolean',
  ...
  return (
    <ComponentPlayground
      ...
      codeTemplate={(v) => {
        ...
        return `<TreeView${props.length ? ' ' + props.join(' ') : ''} />`;
      }}
    />
  );
}
      ...
      props={[
        {
          ...
          type: 'TreeNode[]',
          required: true,
          description: 'Array of tree nodes ({ id, label, icon?, children? }).',
        },
        {
