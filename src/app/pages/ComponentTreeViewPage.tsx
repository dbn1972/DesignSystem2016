...
const TreeViewPreview = ({
  ...
  const renderTreeNode = (node: TreeNode, level: number = 0) => {
    ...
    return (
      <div key={node.key} role="treeitem" aria-expanded={hasChildren ? isExpanded : undefined} aria-level={level + 1} aria-selected={isSelected}>
        <div
          className={'flex items-center gap-2 px-2 rounded cursor-pointer hover:bg-gray-100 transition-colors ' + sizeClasses[size] + (isSelected ? ' bg-blue-50 border-l-2 border-[#005196]' : '')}
        ...
        >
          ...
          {hasChildren ? (
            <button
              className="flex-shrink-0 w-4 h-4 flex items-center justify-center text-gray-600 hover:text-gray-900"
              onClick={(e) => {
                e.stopPropagation();
                toggleExpand(node.key);
              }}
          ...
          {checkable && (
            <input
              ...
              className="flex-shrink-0 w-4 h-4 rounded border-gray-300 text-[#005196] focus:ring-[#005196] cursor-pointer"
          ...
          <span className={'flex-grow ' + (isSelected ? 'font-medium text-[#005196]' : 'text-gray-900')}>
            {node.title}
          </span>
        </div>
  ...
  return (
    <div className="border border-gray-200 rounded-lg p-2 bg-white" role="tree" aria-label="Tree view">
