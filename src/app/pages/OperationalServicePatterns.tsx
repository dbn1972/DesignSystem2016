...
import {  Settings, Calendar, CreditCard, XCircle, Upload, FileCheck, Search, Bell, UserCheck, Save, CheckCircle, AlertCircle, ArrowRight, Clock, HelpCircle, Shield, Globe, Info, Database, Code, Zap, Download, Copy, Check } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-12 py-12">
        ...
        <OperationalServiceCodeDownloads />
...
const OPERATIONALSERVICE_REACT_CODE = `import React, { useState } from 'react';

export default function OperationalServicePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const res = await fetch('/api/operational-service', {
        method: 'POST',
...
const OPERATIONALSERVICE_ANGULAR_CODE = `import { Component } from '@angular/core';
import { CommonModule } from '@angular/common';

@Component({
  selector: 'ux4g-operational-service',
  standalone: true,
...
export class OperationalServiceComponent {
  loading = false;
  submitted = false;
  error = '';
...
const OPERATIONALSERVICE_HTML_CODE = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Operational Service — UX4G</title>
  <style>
...
function OperationalServiceCodeDownloads() {
  const [copiedId, setCopiedId] = useState<string | null>(null);
  const copyToClipboard = (code: string, id: string) => { navigator.clipboard.writeText(code); setCopiedId(id); setTimeout(() => setCopiedId(null), 2000); };
  const downloadCode = (code: string, filename: string) => { const blob = new Blob([code], { type: 'text/plain' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = filename; a.click(); URL.revokeObjectURL(url); };
  const lanes = [
    { key: 'react', title: 'React', desc: 'TypeScript + Hooks', code: OPERATIONALSERVICE_REACT_CODE, filename: 'OperationalServicePage.tsx' },
    { key: 'angular', title: 'Angular', desc: 'Standalone Component', code: OPERATIONALSERVICE_ANGULAR_CODE, filename: 'operational-service.component.ts' },
    { key: 'html', title: 'HTML / CSS / JS', desc: 'No framework needed', code: OPERATIONALSERVICE_HTML_CODE, filename: 'operational-service.html' },
  ];
  return (
    <section id="code-downloads" className="space-y-6 scroll-mt-24 mt-12">
      <div className="border-l-4 border-primary pl-4">
        <h2 className="text-2xl font-bold text-foreground">Code Downloads</h2>
        <p className="text-muted-foreground mt-1">Production-ready Operational Service implementations.</p>
      </div>
