...
const DescriptionListPreview = ({
  ...
  const itemClasses = variant === 'horizontal'
    ? compact
      ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 py-2 px-4'
      : 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 py-4 px-6'
    : compact
      ? 'py-1'
      : 'py-2';
...
const DESCRIPTIONLIST_CONTROLS: PlaygroundControl[] = [
  {
    name: 'layout',
    label: 'Layout',
    type: 'text',
...
function DescriptionListPlayground() {
  return (
    <ComponentPlayground
      ...
      controls={DESCRIPTIONLIST_CONTROLS}
      renderPreview={(v) => (
        <div className="w-full max-w-lg">
          <DescriptionListPreview {...v} />
        </div>
      )}
      codeTemplate={(v) => {
        const props: string[] = [];
        DESCRIPTIONLIST_CONTROLS.forEach((c) => {
          const val = v[c.name];
        ...
        return `<DescriptionList${props.length ? ' ' + props.join(' ') : ''} />`;
      }}
    />
  );
}
      ...
      props={[
        {
          ...
          required: true,
          description: 'Array of items ({ term: string; description: ReactNode }).',
        },
        {
          name: 'layout',
          type: "'vertical' | 'horizontal'",
          required: false,
          description: 'Layout direction for term-description pairs.',
        },
        {
          name: 'striped',
          type: 'boolean',
