...
const OTPINPUT_PLAYGROUND_CONTROLS: PlaygroundControl[] = [
  { name: 'length', label: 'Length', type: 'text', defaultValue: '4' },
...
function OTPInputPlayground() {
  return (
    <ComponentPlayground
      name="OTPInput"
      controls={OTPINPUT_PLAYGROUND_CONTROLS}
      renderPreview={(v) => (
      ...
      codeTemplate={(v) =>
        `<OTPInput ${v.length}${v.disabled ? ' disabled' : ''} />`
      }
    />
  );
}
