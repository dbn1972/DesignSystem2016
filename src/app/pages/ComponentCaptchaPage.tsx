              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-center text-2xl cursor-pointer border border-gray-300 rounded transition-colors ${
                    hasTarget ? 'bg-amber-50 hover:bg-amber-100' : 'bg-muted hover:bg-gray-200'
                  }`}
                >
                  {hasTarget ? '🚦' : ['🏠', '🚗', '🌳', '🏢', '🛣️', '🚶'][i % 6]}
                </div>
...
const CAPTCHA_PLAYGROUND_CONTROLS: PlaygroundControl[] = [
  { name: 'type', label: 'Type', type: 'text', defaultValue: 'image' },
  { name: 'difficulty', label: 'Difficulty', type: 'text', defaultValue: 'medium' },
  { name: 'showRefresh', label: 'Show Refresh', type: 'boolean', defaultValue: true },
];
    ...
    <ComponentDocumentation
      ...
      props={[
        ...
        },
        {
          ...
          type: 'number',
          required: false,
          description: 'Length of the captcha code.',
        },
        {
          name: 'label',
          type: 'string',
          required: false,
          description: 'Label text for the captcha.',
        },
        {
          name: 'errorText',
          type: 'string',
