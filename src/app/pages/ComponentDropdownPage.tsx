  ...
  return (
    <div className={'relative inline-block ' + className} ref={dropdownRef}>
      <button
        ...
        disabled={disabled}
        className={'inline-flex items-center justify-center gap-2 rounded border font-medium transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 min-h-[44px] bg-white text-[#005196] border-[#005196] hover:bg-[#f5f5f5] disabled:opacity-60 disabled:cursor-not-allowed ' + (size === 'sm' ? 'h-8 px-3 text-sm' : size === 'lg' ? 'h-12 px-6 text-base' : 'h-10 px-4 text-sm')}
        aria-haspopup="true"
      ...
      {isOpen && (
        <div
          className={'absolute z-50 mt-1 rounded-md border border-gray-200 bg-white shadow-lg ' + menuWidthClass + ' ' + placementClasses}
          role="menu"
        >
          <div className={'py-1 ' + sizeClasses}>
            {items.map((item: any, index: number) => {
              ...
              const textColorClass = isDanger ? 'text-red-700' : 'text-gray-700';
              const hoverClass = isDanger ? 'hover:bg-red-50 hover:text-red-900' : 'hover:bg-gray-100 hover:text-gray-900';

              return (
                <button
                  key={index}
                  className={'w-full text-left px-3 py-2 transition-colors flex items-center gap-2 ' + textColorClass + ' ' + hoverClass + ' focus:outline-none focus:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed'}
                  role="menuitem"
      ...
      props={[
        {
          ...
          type: 'ReactNode',
          required: true,
          description: 'Element that triggers the dropdown menu. Can be text, icon, or custom component.',
        },
        {
          name: 'items',
          type: 'DropdownItem[]',
          required: true,
          description: 'Array of menu items. Each item can be a standard item, divider, or header. Items support icons, badges, disabled state, and custom click handlers.',
        },
        {
          name: 'placement',
          ...
          default: "'bottom-end'",
          required: false,
          description: 'Position of the dropdown menu relative to trigger. Bottom-end aligns menu to right edge, bottom-start to left edge.',
        },
        {
          name: 'onSelect',
          type: '(item: DropdownItem) => void',
          required: false,
          description: 'Callback fired when menu item is selected. Receives the selected item as argument.',
        },
        {
          name: 'disabled',
          type: 'boolean',
          default: 'false',
          required: false,
          description: 'Whether the dropdown trigger is disabled. Prevents opening the menu.',
        },
        {
          name: 'closeOnSelect',
          type: 'boolean',
          default: 'true',
          required: false,
          description: 'Whether to close the menu after selecting an item. Can be overridden per item.',
        },
        {
          name: 'size',
          type: "'sm' | 'md' | 'lg'",
          default: "'md'",
          required: false,
          description: 'Size of the dropdown menu. Affects text size and menu width.',
        },
        {
          name: 'className',
          type: 'string',
      ...
      useCases={[
        {
          ...
          example: 'Header component with user avatar and dropdown containing Profile, Settings, Help Center, and Sign Out options.',
        },
        {
          ...
          example: 'Application list with action dropdowns offering View Details, Download PDF, Edit Application, Withdraw, and Delete options.',
        },
        {
          title: 'Language Selector',
          description: 'Dropdown menu for selecting interface language in multilingual government services.',
          example: 'Header language selector with options for English, French, Spanish, and other supported languages with flag icons.',
        },
        {
          title: 'Notification Center',
          description: 'Dropdown displaying recent notifications and alerts with badge count indicator.',
          example: 'Bell icon with notification count badge opening dropdown with recent alerts, messages, and "View All" link.',
        },
        {
          title: 'Export Options',
          description: 'Menu for exporting data and reports in multiple formats (PDF, CSV, Excel, JSON).',
          example: 'Export button in data table toolbar with dropdown offering different export format options.',
        },
        {
          title: 'Bulk Actions',
          description: 'Action menu for performing operations on multiple selected items in tables or lists.',
          example: 'Table toolbar with "Bulk Actions" dropdown containing Approve Selected, Reject Selected, Export Selected, and Delete Selected.',
        },
        {
          title: 'Document Management',
          description: 'Context menu for managing uploaded documents with view, download, share, and delete actions.',
          example: 'Document list with three-dot menu icons offering View, Download, Share Link, Move to Folder, and Delete operations.',
        },
        {
          title: 'Status Filter',
          description: 'Filter dropdown for selecting application or request status in dashboard views.',
          example: 'Dashboard filter bar with status dropdown containing All, Pending, Approved, Rejected, and In Review options with checkmarks.',
        },
      ]}
    />
  );
