    ...
    <ComponentPlayground
      ...
      renderPreview={(v) => (
        <div className="w-full max-w-lg w-full flex items-center justify-center">
          <div className="w-full space-y-4"><p className="text-sm text-muted-foreground">Content above</p><DividerPreview orientation={v.orientation} variant={v.variant} /><p className="text-sm text-muted-foreground">Content below</p></div>
        </div>
      )}
      codeTemplate={(v) =>
        `<Divider ${v.orientation} ${v.variant} />`}
    />
  );
}
