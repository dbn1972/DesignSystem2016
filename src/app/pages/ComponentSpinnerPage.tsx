...
const SPINNER_PLAYGROUND_CONTROLS: PlaygroundControl[] = [
  { name: 'size', label: 'Size', type: 'select', defaultValue: 'sm', options: ['sm', 'md', 'lg'] },
  { name: 'label', label: 'Label', type: 'text', defaultValue: 'Loading...' },
  { name: 'overlay', label: 'Overlay mode', type: 'boolean', defaultValue: false },
];
...
function SpinnerPlayground() {
  return (
    <ComponentPlayground
      name="Spinner"
      controls={SPINNER_PLAYGROUND_CONTROLS}
      renderPreview={(v) => (
        <SpinnerPreview size={v.size} label={v.label} />
      )}
      codeTemplate={(v) =>
        `<Spinner size="${v.size}"${v.label !== 'Loading...' ? ` label="${v.label}"` : ''}${v.overlay ? ' overlay' : ''} />`
      }
    />
  );
}
