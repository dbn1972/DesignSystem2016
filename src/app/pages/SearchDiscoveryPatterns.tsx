...
import {  Search, Filter, AlertCircle, TrendingUp, Clock, Compass, List, ArrowRight, CheckCircle, Zap, Download, Copy, Check } from "lucide-react";
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-8 lg:px-12 py-16">
          ...
          <div className="flex items-start justify-between">
            ...
            <div className="flex flex-col gap-3">
              <div className="px-8 py-4 bg-purple-50 dark:bg-purple-900/200 text-white rounded-lg font-bold text-center shadow-lg">
                DISCOVERY SYSTEM
              </div>
...
const SEARCHDISCOVERY_REACT_CODE = `import React, { useState } from 'react';
...
export default function SearchDiscoveryPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const res = await fetch('/api/search-discovery', {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <div className="w-full max-w-lg bg-card border border-border rounded-2xl p-8 shadow-sm">
        <h1 className="text-2xl font-bold text-foreground mb-2">Search Discovery</h1>
...
const SEARCHDISCOVERY_ANGULAR_CODE = `import { Component } from '@angular/core';
import { CommonModule } from '@angular/common';

@Component({
  selector: 'ux4g-search-discovery',
  standalone: true,
  imports: [CommonModule],
  template: \`
    <div class="min-h-screen flex items-center justify-center bg-background p-4">
      <div class="w-full max-w-lg bg-card border border-border rounded-2xl p-8 shadow-sm">
        <h1 class="text-2xl font-bold text-foreground mb-2">Search Discovery</h1>
...
export class SearchDiscoveryComponent {
  loading = false;
  submitted = false;
  error = '';
...
const SEARCHDISCOVERY_HTML_CODE = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Search Discovery — UX4G</title>
  <style>
...
function SearchDiscoveryCodeDownloads() {
  const [copiedId, setCopiedId] = useState<string | null>(null);
  const copyToClipboard = (code: string, id: string) => { navigator.clipboard.writeText(code); setCopiedId(id); setTimeout(() => setCopiedId(null), 2000); };
  const downloadCode = (code: string, filename: string) => { const blob = new Blob([code], { type: 'text/plain' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = filename; a.click(); URL.revokeObjectURL(url); };
  const lanes = [
    { key: 'react', title: 'React', desc: 'TypeScript + Hooks', code: SEARCHDISCOVERY_REACT_CODE, filename: 'SearchDiscoveryPage.tsx' },
    { key: 'angular', title: 'Angular', desc: 'Standalone Component', code: SEARCHDISCOVERY_ANGULAR_CODE, filename: 'search-discovery.component.ts' },
    { key: 'html', title: 'HTML / CSS / JS', desc: 'No framework needed', code: SEARCHDISCOVERY_HTML_CODE, filename: 'search-discovery.html' },
  ];
  return (
    <section id="code-downloads" className="space-y-6 scroll-mt-24 mt-12">
      <div className="border-l-4 border-primary pl-4">
        <h2 className="text-2xl font-bold text-foreground">Code Downloads</h2>
        <p className="text-muted-foreground mt-1">Production-ready Search Discovery implementations.</p>
      </div>
