...
const DIGITALSIGNATURE_PLAYGROUND_CONTROLS: PlaygroundControl[] = [
  { name: 'type', label: 'Type', type: 'text', defaultValue: 'dsc' },
  { name: 'showTimestamp', label: 'Show Timestamp', type: 'boolean', defaultValue: true },
...
function DigitalSignaturePlayground() {
  return (
    <ComponentPlayground
      ...
      controls={DIGITALSIGNATURE_PLAYGROUND_CONTROLS}
      renderPreview={(v) => (
        <div className="w-full flex items-center justify-center">
          <DigitalSignaturePreview type={v.type} />
        </div>
      )}
      codeTemplate={(v) =>
        `<DigitalSignature ${v.type} />`
      }
    />
  );
}
