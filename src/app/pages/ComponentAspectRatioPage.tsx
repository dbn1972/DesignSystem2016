...
const ASPECTRATIO_PLAYGROUND_CONTROLS: PlaygroundControl[] = [
  { name: 'ratio', label: 'Ratio', type: 'text', defaultValue: '16/9' },
  { name: 'showBorder', label: 'Show Border', type: 'boolean', defaultValue: true },
  { name: 'rounded', label: 'Rounded', type: 'boolean', defaultValue: true },
];
...
function AspectRatioPlayground() {
  return (
    <ComponentPlayground
      name="AspectRatio"
      controls={ASPECTRATIO_PLAYGROUND_CONTROLS}
      renderPreview={(v) => (
        <div className="w-full flex items-center justify-center">
          <AspectRatioPreview ratio={v.ratio} />
        </div>
      )}
      codeTemplate={(v) =>
        `<AspectRatio ${v.ratio} />`
