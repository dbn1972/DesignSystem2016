...
const CHATBOT_PLAYGROUND_CONTROLS: PlaygroundControl[] = [
  { name: 'position', label: 'Position', type: 'text', defaultValue: 'bottom-right' },
  { name: 'minimized', label: 'Minimized', type: 'boolean', defaultValue: false },
  { name: 'showAvatar', label: 'Show Avatar', type: 'boolean', defaultValue: true },
];
...
function ChatbotPlayground() {
  return (
    <ComponentPlayground
      name="Chatbot"
      controls={CHATBOT_PLAYGROUND_CONTROLS}
      renderPreview={(v) => (
        <div className="w-full flex items-center justify-center">
          <ChatbotPreview position={v.position} minimized={v.minimized} botName="UX4G Assistant" greeting="How can I help?" />
        </div>
      )}
      codeTemplate={(v) =>
        `<Chatbot ${v.position} />`
